...
import { MessageSquare, Trash2, X, User } from "lucide-react";
import api from "../api/axios";

export default function CommentModal({ open, onClose, itemId, itemName }) {
  ...
  const loadComments = async () => {
    ...
    try {
      setLoading(true);
      setError("");
      const response = await api.get(`/admin/comments/${itemId}`);
          ...
          <div className="flex-1 overflow-y-auto p-5 pretty-scroll">
            {loading ? (
              ...
              <div className="flex h-full flex-col items-center justify-center py-10 text-center">
                ...
                <p className="mt-1 text-xs text-slate-400">Be the first to see what users say!</p>
              </div>
            ) : (
